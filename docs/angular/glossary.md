...
[A][aioguideglossarya] [B][aioguideglossaryb]
...
Подобно [JavaScript-модулю][aioguideglossarymodule], NgModule может экспортировать функциональность для использования другими NgModules и импортировать публичную функциональность из других NgModules. В метаданных класса NgModule собраны компоненты, директивы и трубы, которые использует приложение, а также список импорта и экспорта.
...
Класс, которому предшествует декоратор `@Pipe{}` и который определяет функцию, преобразующую входные значения в выходные для отображения в [view][aioguideglossaryview]. Angular определяет различные трубы, и вы можете определить новые трубы.
...
Конфигурация из [компонентов][aioguideglossarycomponent], [директив][aioguideglossarydirective] и [труб][aioguideglossarypipe], указывающая, что этот класс может быть импортирован напрямую без объявления его в каком-либо [NgModule][aioguideglossaryngmodule].

Автономные компоненты, директивы и трубы отличаются от нестандартных тем, что:
