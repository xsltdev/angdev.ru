...
API _directive composition API_ позволяет применять директивы к главному элементу компонента изнутри TypeScript-класса компонента.
...
и выходы не отображаются как часть публичного API компонента.
...
Директивы хоста проходят через тот же жизненный цикл, что и компоненты и директивы, используемые непосредственно в шаблоне. Однако директивы хоста всегда выполняют свой конструктор, крючки жизненного цикла и привязки _до_ компонента или директивы, к которым они применяются.
...
4.  `Tooltip` получает входные данные (`ngOnInit`)
...
В этом примере объявлен компонент пользовательского флажка, который включает пять директив хоста. Это означает, что Angular будет создавать шесть объектов при каждом рендеринге `CustomCheckbox` - один для компонента и по одному для каждой директивы-хоста. Для нескольких флажков на странице это не создаст никаких существенных проблем. Однако, если на вашей странице отображаются _сотни_ флажков, например, в таблице, тогда вы можете начать ощущать влияние дополнительных выделений объектов. Всегда проверяйте профиль вашего приложения, чтобы определить правильный шаблон композиции для вашего случая использования.
