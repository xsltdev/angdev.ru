...
-   Не проверяются типы `#refs`, результаты работы труб, тип `$event` в привязках событий.
...
-   Локальные ссылки на директивы и трубы имеют правильный тип (за исключением любых общих параметров, которые будут иметь тип `any`)
